...
def ViT_large_patch32_384(pretrained=False, use_ssld=False, **kwargs):
    ...
    return model
paddle_vit = ViT_small_patch16_224(qkv_bias=True, epsilon=1e-6)


def paddle_loss(paddle_out, label=None):
        return nn.CrossEntropyLoss()(paddle_out, label)
...
torch_model = VisionTransformer(patch_size=16,
                                embed_dim=768,
                                depth=12,
                                num_heads=8,
                                mlp_ratio=4.0,)
...
import paddle
import torch
d = paddle.randn((3,3,224,224))
torch_input = torch.randint(0, 1000, (3,))
paddle_input = paddle.to_tensor(torch_input.numpy())
align = ModelAlign(paddle_vit, 
                   torch_model, 
                   input_data=d,
...
align.forward()
align.backward(paddle_input={'label':paddle_input}, torch_input={'label':torch_input})
